...
class GeminiService:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize Gemini service: {str(e)}", exc_info=True
            )
            raise

    async def get_travel_info(
        self, query: str, destination: str, origin: str = None
    ) -> dict[str, Any]:
        ...
        try:
            logger.info(
                f"Generating travel info for destination: {destination}, origin: {origin}"
            )
